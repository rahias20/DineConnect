...
import 'package:dine_connect/view/login_page.dart'; // Update with the correct path
...
void main() {
  ...
  testWidgets('Displays error message for invalid email and password',
    ...
    await tester.pumpWidget(MaterialApp(
      home: MediaQuery(
        data: const MediaQueryData(
            size:
                Size(1080, 1920)), // Make sure this size fits all your widgets
    ...
    final Finder loginButtonFinder =
        find.widgetWithText(GestureDetector, 'Login');
    await tester.ensureVisible(loginButtonFinder);
    await tester.pumpAndSettle(); // Wait for any animations to complete
    await tester.tap(loginButtonFinder);
    await tester
        .pumpAndSettle(); // Allow time for the onTap event to be processed

    // Check for the expected error messages.
    expect(find.text('Please enter an email'),
        findsOneWidget); // Confirm the invalid email message is shown
    expect(find.text('Please enter a password'),
        findsOneWidget); // Confirm the password message is shown.
